...
compute_stats <- function(data) {
  data[, .(
    ...
    phi.plus6_lin_mean = mean(phi_plus6),
    phi.plus6_lin_median = median(phi_plus6),
    phi.plus6_lin_ci_width = quantile(phi_plus6, 0.975) - quantile(phi_plus6, 0.025),
    phi.plus6_circ_mean = (mean.circular(phi_plus6) %% (2 * pi)) %>% as.numeric(),
    phi.plus6_circ_median = (median.circular(phi_plus6) %% (2 * pi)) %>% as.numeric(),
    phi.plus6_circ_ci_width = (
      (quantile.circular(phi_plus6, 0.975) - quantile.circular(phi_plus6, 0.025)) %% (2 * pi)
    ) %>% as.numeric(),
    ## Phi with half cycle (12h) offset
    phi.plus12_lin_mean = mean(phi_plus12),
    phi.plus12_lin_median = median(phi_plus12),
    phi.plus12_lin_ci_width = quantile(phi_plus12, 0.975) - quantile(phi_plus12, 0.025),
    phi.plus12_circ_mean = (mean.circular(phi_plus12) %% (2 * pi)) %>% as.numeric(),
    phi.plus12_circ_median = (median.circular(phi_plus12) %% (2 * pi)) %>% as.numeric(),
    phi.plus12_circ_ci_width = (
      (quantile.circular(phi_plus12, 0.975) - quantile.circular(phi_plus12, 0.025)) %% (2 * pi)
    ) %>% as.numeric()
  )]
}
...
saveRDS(d_level1, "hap_level1.rds")
