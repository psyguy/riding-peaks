...
p_1 <- f_plot_correlation_demonstration(data.frame(phi = phi_sim,
                                            z = z_sim),
                                        "Raw simulated data")

p_2 <- f_plot_correlation_demonstration(data.frame(phi = (phi_sim + 0.3*pi) %% (2*pi),
                                            z = z_sim),
                                        expression("Circular variable shifted by" ~ +0.3 * pi))

p_3 <- f_plot_correlation_demonstration(data.frame(phi = (phi_sim + 0.8*pi) %% (2*pi),
                                            z = z_sim),
                                        expression("Circular variable shifted by" ~ +0.8 * pi))

p_4 <- f_plot_correlation_demonstration(data.frame(phi = phi_sim,
                                            z = z_sim + 15),
                                        "Linear variable shifted by +15")

p_5 <- f_plot_correlation_demonstration(data.frame(phi = phi_sim,
                                            z = z_sim - 15),
                                        "Linear variable shifted by -15")
