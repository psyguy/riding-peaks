...
p_circle <- ggplot() +
  ...
  scale_fill_manual(values = c("circular" = "cornflowerblue",
                               "linear" = "brown1")) +
  scale_color_manual(values = c("circular" = "cornflowerblue",
                                "linear" = "brown1")) +
...
(p_circle / plot_spacer() / p_hist) +
  plot_layout(heights = c(1, 0.1, 0.5))
...
Now we make the plots for the distributions of estimated mean, median, and 95%CI width for datasets, for linear or circular methods. For that we first need to reahape the dataframe:
...
ggsave("distribution-l1-estimates.pdf",
       width = 25,
       height = 12.5,
       units = "cm")
```
