...
We now make two dataframes: One for ESM variables, and one with only the baseline variables (which we need `bl_pa` and `bl_na` here). For each of the datasets and items, run the respective lines.
