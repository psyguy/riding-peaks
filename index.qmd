...
We now make two dataframes: One for ESM variables, and one with only the baseline variables (which we need `bl_pa` and `bl_na` here).
For each of the datasets and items, run the respectove lines.
...
From now on, we move on with the estimates of the second model; for the first model, remove `sigma`...
