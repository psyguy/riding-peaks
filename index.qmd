...
f_plot_circular_linear <- function(phi, title = "Circular and linear histograms") {
  ...
  p_hist <- ggplot(data.frame(x = phi), aes(x = x)) +
    geom_histogram(aes(y = ..ncount..), bins = 120, alpha = 0.6, fill = "azure4") +
    geom_segment(
      data = d_estimates,
    ...
    geom_rect(
      data = d_estimates %>%
        ...
        distinct(),
      aes(xmin = ci_2.5, xmax = ci_97.5, ymin = 0, ymax = r^3 * 0.4, fill = type),
...
p_hdr_pa <- el1.5e_level1.5 %>% 
    filter(item == "pa") %>% 
    f_plot_hdr("co_median", "si_median",
               title = "Dataset 1")

p_hdr_fitbit <- el1.5e_level1.5 %>% 
    filter(item == "fitbit") %>% 
    f_plot_hdr("co_median", "si_median",
               title = "Dataset 2")
...
el2 %>%
  ...
  mutate(mm = median (value),
         ...
         par1 = case_when(par1 == "amp" ~ "Amplitude",
                          ...
                          TRUE ~ par1) %>% 
           factor(levels = c("Amplitude", "MESOR", "log(SD)"))) %>%
  mutate(measure = "") %>%
