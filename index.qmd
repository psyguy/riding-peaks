...
For the second model, we also extract `sigma2` (i.e., ${\sigma_i^s}^2$ from `sigmaIntercept^2`, and also make `log_sigma2` (by taking the log of `sigma2`).[^1]

[^1]: I have been under the impression that `sigmaIntercept` (initially stored in `r_id__sigma[1,Intercept]`) were $\ln(\sigma_i^s)$; but looking at the estimates, they appear to be the residual variance of samples (i.e., ${\sigma_i^s}^2$,).
    So now I store them as `sigma2`, and make another variable `log_sigma2 = log(sigma2)` to have something perhaps comparable to Mplus.
    To revert back to my initial impression, uncomment the respective lines in the `mutate` function at the end.
...
To get person-specific summary statistics, we do calculations across iterations (i.e., only `group_by(id) %>% summarize(...)`) to get mean, median, and 95%CI width of our estimated parameters ... , and for , once incorrectly (using linear methods) and once correctly (using circular methods).
...
ests_level1 <- d_draws %>%
  ...
  summarise(
    bind_cols(
      ...
      compute_linear_stats(cur_data(), "log_sigma2"),
...
compute_stats <- function(dt) {
  dt[, {
    ...
    linear_stats <- unlist(lapply(
      c("mesor", "si", "co", "amp", "sigma2", "log_sigma2", "phi", "phi.begins6", "phi.begins12"),
...
ests_level1 <- rbindlist(mclapply(data_split, compute_stats, mc.cores = detectCores() - 1))
```
...
cors_lin_angle <- d_draws %>%
  ...
  summarise(
    crossing(
      linear_variable = c('mesor', 'log_sigma2', 'sigma2', 'amp'),
    ...
    summarise(
        pair_data = list(
            tibble(
                ...
                par2 = c("amp", "log_sigma2", "sigma2", "log_sigma2", "sigma2")
            )
        )
    ) %>%
    unnest(pair_data) %>%
...
ests_level2 <- cors_lin_angle %>%
    ...
    group_by(what) %>% 
    mutate(
        correlation_type = str_split(what, "_")[[1]][1],
        measure = str_split(what, "_")[[1]][2],
        .before = value
    ) %>% 
  ungroup() %>% 
  select(-what)
