...
We make a new dataframe `d` by first filtering the data to only scores from one item denoted by `item_`;
we concider `hap`, `sad`, `pa`, and `na` for `item_`, starting by `item_ <- "hap"`.
